...
public class Collector implements ManagerInterface, Serializable{
	 ...
	 * @return boolean
	 */
	public boolean isExpenseOverLimit(Expense expense){
		Date date = expense.getDate();
		double totalValue = 0;
		Format formatter = null;
		for(Entry<String, Double> limit: expensesLimits.entrySet()){
			switch(limit.getKey()){
			case "month":
				if(limit.getValue() > 0){
					formatter = new SimpleDateFormat("mm-yyyy");
					totalValue = getExpensesTotalValue(formatter.format(date));
					if(totalValue > 0 && totalValue > limit.getValue()){
						return true;
					}
				}
				break;
	...
	/**
	 * Gte expenses total value for a date input, accepted formats: yyyy and mm-yyyy
	 * @param dateInput
	 * @return double
